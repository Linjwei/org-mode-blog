...
The destination  is the =:publishing-directory= which is read from the environment variable =PUBLISHING_DIRECTORY=. It can be a local directory like =~/public_html= or a remote location like =/ssh:user@server:directory=.
