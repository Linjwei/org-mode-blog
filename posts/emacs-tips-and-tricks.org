...
...which maximizes the current startup frame and also ensures that all future frames are maximized by modifying =default-frame-alist=.

*** New command-line option

There has been added a new command-line option -mm (--maximized) which maximizes the initial frame.
